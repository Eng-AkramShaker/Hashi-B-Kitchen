...
import 'package:face_auth_flutter/data/models/print/print_sales_invoice.dart';
...
import 'package:face_auth_flutter/data/services/orders/order_api.dart';
import 'package:face_auth_flutter/data/services/payment/payment_api.dart';
import 'package:face_auth_flutter/data/services/serial/serial_number_api.dart';
...
// remove item ==================================================================================
  ...
  Send_Total_Sales_Invoice({
    ...
    if (items.isEmpty) {
      ...
      await Api_Send_Sales_Invoice(context, user.branch!, Date_Now, Name_Customer, Send_items_Api, "Cash", 10, order_options_method);
  ...
  Fetch_Payment_Method(context) async {
    List_Type_Payment = await Api_Get_Payment_Method(context);
  ...
  Fetch_Order_Options_Method(context) async {
    await Api_Get_Order_Options_Method(context).then((value) {
  ...
  Fetch_Serial_Number(BuildContext context) async {
    List_Serial_Number = [];
    ModelSerialNumber? modelSerialNumber = await Api_Get_Serial_Number_Invoice(context);
