...
class Draft_Screen extends StatefulWidget {
  ...
  @override
  _Draft_ScreenState createState() => _Draft_ScreenState();
}
      ...
      builder: (context, provCategory, proInvoice, provLogin, proKitchen, child) {
        return Padding(
          ...
          child: CustomScrollView(
            slivers: [
              proKitchen.data_Draft_Kitchen != null
                  ? SliverGrid(
                      ...
                      delegate: SliverChildBuilderDelegate(
                        childCount: proKitchen.getFilteredDeliveries(proKitchen.data_Draft_Kitchen!).length,
                        (context, index) {
                          final item = proKitchen.getFilteredDeliveries(proKitchen.data_Draft_Kitchen!)[index];
                          return Padding(
                            padding: const EdgeInsets.all(8.0),
            ...
            children: [
              ...
              SizedBox(
                ...
                child: Card(
                  color: Colors.green,
                  child: IconButton(
                    icon: const Icon(color: Colors.white, size: 19, Icons.check),
                    onPressed: () async {
                      // Action for the button

                      await pro_Kitchen.Check_Submit_Kitchen(context, item.salesInvoice!.toString());
                    },
                  ),
                ),
              ),
